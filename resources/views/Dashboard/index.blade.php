...
<style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .search-wrap input {
        width: 100%; padding: 7px 10px 7px 32px; font-size: .82rem;
    ...
    .search-wrap input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(30,95,168,.12); background-color: #fff; }
    ...
    .u-info strong { display: block; font-size: .82rem; font-weight: 600; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .u-info span   { font-size: .72rem; color: var(--muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }

    .rbadge { flex-shrink: 0; font-size: .62rem; font-weight: 700; padding: 2px 7px; border-radius: 20px; }
    .rbadge.sa { background: var(--danger-l); color: var(--danger); }
    .rbadge.ad { background: var(--primary-l); color: var(--primary); }

    .no-results { padding: 24px; text-align: center; font-size: .82rem; color: var(--muted); display: none; }

    /* Panel derecho */
    .main-panel { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow); overflow: hidden; }

    .state-empty {
        display: flex; flex-direction: column; align-items: center; justify-content: center;
        min-height: 400px; padding: 48px 32px; text-align: center; color: var(--muted);
    }
    ...
    .rol-opt {
        position: relative; border: 1.5px solid var(--border); border-radius: 8px;
    ...
    .rol-icon { width: 32px; height: 32px; border-radius: 7px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    ...
    .rol-check {
        margin-left: auto; flex-shrink: 0; width: 16px; height: 16px; border-radius: 50%;
    ...
    .rol-opt.sel-admin .rol-check svg, .rol-opt.sel-sa .rol-check svg { opacity: 1; }

    .perms-section { animation: fadeUp .2s ease; }
    @keyframes fadeUp { from { opacity:0; transform:translateY(5px); } to { opacity:1; transform:translateY(0); } }

    .perms-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
    .perms-actions { display: flex; gap: 10px; }
    .link-btn { font-size: .75rem; font-weight: 500; color: var(--primary); background: none; border: none; cursor: pointer; padding: 0; transition: opacity .15s; }
    .link-btn:hover { opacity: .7; }
    .link-btn.red { color: var(--danger); }

    .perms-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(175px, 1fr)); gap: 7px; }
    ...
    /* Tabs */
    .tab-bar { display: flex; gap: 3px; background: #f3f4f6; padding: 3px; border-radius: 9px; width: fit-content; margin-bottom: 18px; }
    .tab-btn { padding: 6px 18px; border-radius: 7px; font-size: .82rem; font-weight: 600; color: var(--muted); cursor: pointer; border: none; background: none; transition: all .2s; }
    .tab-btn.active { background: #fff; color: var(--primary); box-shadow: 0 1px 4px rgba(0,0,0,.09); }
    .tab-pane { display: none; }
    .tab-pane.active { display: block; animation: fadeUp .2s ease; }

    /* Tabla comparativa */
    .cmp-table { width: 100%; border-collapse: collapse; font-size: .8rem; }
    .cmp-table th { padding: 8px 14px; text-align: left; font-size: .68rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; color: var(--muted); background: var(--bg); border-bottom: 2px solid var(--border); }
    ...
    .badge-cfg { display: inline-flex; padding: 1px 8px; border-radius: 20px; background: var(--primary-l); color: var(--primary); font-size: .68rem; font-weight: 700; }

    /* Alertas flash */
    .flash-ok { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 8px; background: #f0fdf4; border: 1px solid #bbf7d0; color: #15803d; font-size: .83rem; font-weight: 500; margin-bottom: 16px; }
    .flash-err { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 8px; background: var(--danger-l); border: 1px solid #fecaca; color: var(--danger); font-size: .83rem; font-weight: 500; margin-bottom: 16px; }
    .flash-ok svg, .flash-err svg { width: 16px; height: 16px; flex-shrink: 0; }

    /* Footer form */
    .form-footer { display: flex; align-items: center; justify-content: flex-end; gap: 8px; padding: 12px 20px; border-top: 1px solid var(--border); background: var(--bg); }
    .btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 18px; border-radius: 8px; font-size: .82rem; font-weight: 600; cursor: pointer; border: none; transition: all .2s; }
    .btn svg { width: 14px; height: 14px; }
    .btn-ghost   { background: transparent; color: var(--muted); border: 1px solid var(--border); }
    .btn-ghost:hover { background: #f3f4f6; }
    .btn-primary { background: var(--primary); color: #fff; box-shadow: 0 2px 6px rgba(30,95,168,.25); }
...
<div style="padding: 1.5rem;">
    ...
    <div class="tab-pane" id="tab-resumen">
                    ...
                    <div class="card-header border-0 py-4 px-4" style="background:linear-gradient(135deg,#ef4444 0%,#f97316 100%);">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-3">
                                <div style="width:50px;height:50px;background:white;border-radius:10px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.15);">
                                    <i class="fas fa-user-shield" style="font-size:1.4rem;color:#ef4444;"></i>
                                </div>
                                <div>
                                    <h5 class="text-white fw-bold mb-0">Super Administrador</h5>
                                    <small class="text-white" style="opacity:.85;">Acceso Total al Sistema</small>
                                </div>
                            </div>
                    ...
                    <div class="card-body p-4">
                        <div class="card border-0 mb-3" style="background:#f9fafb;border-radius:10px;">
                            <div class="card-body p-3">
                                <h6 class="fw-bold mb-3" style="color:#003b73;"><i class="fas fa-bolt me-2 text-warning"></i>Capacidades Especiales</h6>
                                <ul class="list-unstyled mb-0">
                                    <li class="d-flex align-items-start gap-2 mb-2"><i class="fas fa-check-circle text-success mt-1 flex-shrink-0"></i><span class="text-muted" style="font-size:.9rem;">Acceso completo a <strong>todas las funciones</strong></span></li>
                                    <li class="d-flex align-items-start gap-2 mb-2"><i class="fas fa-check-circle text-success mt-1 flex-shrink-0"></i><span class="text-muted" style="font-size:.9rem;">Puede <strong>crear, editar y eliminar administradores</strong></span></li>
                                    <li class="d-flex align-items-start gap-2 mb-2"><i class="fas fa-check-circle text-success mt-1 flex-shrink-0"></i><span class="text-muted" style="font-size:.9rem;">Asigna y modifica <strong>permisos de otros usuarios</strong></span></li>
                                    <li class="d-flex align-items-start gap-2 mb-2"><i class="fas fa-check-circle text-success mt-1 flex-shrink-0"></i><span class="text-muted" style="font-size:.9rem;">Acceso a <strong>configuración del sistema</strong></span></li>
                                    <li class="d-flex align-items-start gap-2 mb-0"><i class="fas fa-check-circle text-success mt-1 flex-shrink-0"></i><span class="text-muted" style="font-size:.9rem;">Visualiza <strong>todas las estadísticas y reportes</strong></span></li>
                                </ul>
                            </div>
                        </div>
                        <div class="alert border-0 mb-0" style="background:rgba(239,68,68,.08);border-left:4px solid #ef4444 !important;border-radius:8px;">
                            <div class="d-flex align-items-start gap-2">
                                ...
                                <div>
                                    <p class="fw-semibold text-danger mb-1" style="font-size:.85rem;">Importante</p>
                                    <p class="text-danger mb-0" style="font-size:.8rem;opacity:.85;">Este rol debe ser asignado solo a usuarios de máxima confianza.</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-6">
                <div class="card border-0 shadow-sm h-100" style="border-radius:12px;border:2px solid #93c5fd !important;overflow:hidden;">
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <small class="text-white" style="opacity:.85;">Permisos Personalizables</small>
                                </div>
                            </div>
                    ...
                    <div class="card-body p-4">
                        <div class="card border-0 mb-3" style="background:#f9fafb;border-radius:10px;">
                            <div class="card-body p-3">
                                <h6 class="fw-bold mb-1" style="color:#003b73;"><i class="fas fa-clipboard-list me-2" style="color:#4ec7d2;"></i>Permisos Disponibles</h6>
                                <p class="text-muted mb-3" style="font-size:.8rem;">El Super Admin puede asignar los siguientes permisos:</p>
                                <div class="d-flex flex-column gap-2">
                                    ...
                                    <div class="d-flex align-items-center gap-2 p-2 rounded" style="background:white;border:1px solid #e2e8f0;">
                                        ...
                                        <span style="font-size:.875rem;color:#374151;">{{ $nombre }}</span>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="alert border-0 mb-0" style="background:rgba(245,158,11,.08);border-left:4px solid #f59e0b !important;border-radius:8px;">
                            <div class="d-flex align-items-start gap-2">
                                ...
                                <div>
                                    <p class="fw-semibold text-warning mb-1" style="font-size:.85rem;">Restricciones</p>
                                    <ul class="mb-0 ps-3" style="font-size:.8rem;color:#92400e;">
        ...
        {{-- Tabla comparativa --}}
        <div class="main-panel overflow-hidden mb-4">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    userItems.forEach(item => {
        item.addEventListener('click', function () {
            ...
            document.getElementById('inputAdminId').value = id;
            ...
            const badge = document.getElementById('pRolBadge');
            badge.textContent = rol === 'super_admin' ? 'SA' : 'Admin';
            badge.className   = 'rbadge ' + (rol === 'super_admin' ? 'sa' : 'ad');

            // Seleccionar rol
            document.getElementById(rol === 'super_admin' ? 'rolSA' : 'rolAdmin').checked = true;
            updateRolUI(rol);
    ...
    function updateRolUI(rol) {
        ...
        optAdmin.classList.toggle('sel-admin', rol === 'admin');
        optSA.classList.toggle('sel-sa', rol === 'super_admin');

        permsSection.classList.toggle('hidden', rol === 'super_admin');
        saAlert.classList.toggle('hidden', rol !== 'super_admin');
    }

    document.getElementById('rolAdmin').addEventListener('change', () => updateRolUI('admin'));
    document.getElementById('rolSA').addEventListener('change',    () => updateRolUI('super_admin'));
    ...
    document.getElementById('btnAll').addEventListener('click', () => {
        document.querySelectorAll('.perm-cb').forEach(cb => { cb.checked = true; cb.closest('.perm-item').classList.add('on'); });
    });
    document.getElementById('btnNone').addEventListener('click', () => {
        document.querySelectorAll('.perm-cb').forEach(cb => { cb.checked = false; cb.closest('.perm-item').classList.remove('on'); });
    });
    ...
    // ── Si había old('admin_id'), activar panel ──
    const oldId = document.getElementById('inputAdminId').value;
    if (oldId) {
        const item = document.querySelector(`.user-item[data-id="${oldId}"]`);
        if (item) item.click();
    }
});
</script>
