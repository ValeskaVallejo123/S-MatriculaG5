...
.e-banner-id   {
    ...
    font-size: .78rem; font-weight: 700; color: #003b73; white-space: nowrap;
}
...
/* 4 columnas para aprovechar el ancho completo */
.e-grid4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.25rem; }
@media(max-width:900px) { .e-grid4 { grid-template-columns: 1fr 1fr; } }
@media(max-width:540px) { .e-grid4 { grid-template-columns: 1fr; } }
...
.e-input.is-invalid, .e-select.is-invalid, .e-textarea.is-invalid { border-color: #ef4444; }
...
.e-codigo-wrap .e-input {
    ...
    font-family: 'Courier New', monospace; font-weight: 700; color: #00508f; letter-spacing: .05em;
}
.e-codigo-btn {
...
/* Areas: mas columnas disponibles con ancho completo */
.areas-grid {
...
.e-actions {
    ...
    padding: 1rem 1.25rem; border-top: 1px solid #f1f5f9; background: #fafafa;
}
.e-btn-save {
...
@section('content')
<div class="edit-wrap container-fluid px-4">
        ...
        <div class="e-card">
            ...
            <div class="e-banner">
                ...
                <div>
                    ...
                    <div class="e-banner-sub">
                        C&oacute;digo: <strong>{{ $materia->codigo }}</strong>
                        &nbsp;&middot;&nbsp; {{ ucfirst($materia->nivel) }}
                    </div>
                </div>
            ...
            <div class="e-card-body">
                ...
                {{-- Seccion 1: Identificacion -- 4 columnas --}}
                <div class="e-section-title">
                    <i class="fas fa-id-card"></i> Identificaci&oacute;n
                </div>
                ...
                <div class="e-grid4">
                    ...
                    <div class="e-field">
                        ...
                        <select name="nivel" class="e-select @error('nivel') is-invalid @enderror"
                                id="selNivel" required>
                            <option value="">Seleccione...</option>
                            <option value="primaria"   {{ old('nivel', $materia->nivel) == 'primaria'   ? 'selected' : '' }}>Primaria (1&deg; - 6&deg;)</option>
                            <option value="secundaria" {{ old('nivel', $materia->nivel) == 'secundaria' ? 'selected' : '' }}>Secundaria (7&deg; - 9&deg;)</option>
                        </select>
                        @error('nivel')<div class="e-error">{{ $message }}</div>@enderror
                        <span class="e-hint">Determina las &aacute;reas disponibles</span>
                    </div>

                    {{-- Nombre --}}
                    <div class="e-field">
                        <label class="e-label">Nombre <span class="req">*</span></label>
                        <input type="text" name="nombre" id="inpNombre"
                               value="{{ old('nombre', $materia->nombre) }}"
                               class="e-input @error('nombre') is-invalid @enderror"
                               placeholder="Ej: Matem&aacute;ticas" required>
                        @error('nombre')<div class="e-error">{{ $message }}</div>@enderror
                    </div>

                    {{-- Codigo --}}
                    <div class="e-field">
                        <label class="e-label">C&oacute;digo <span class="req">*</span></label>
                        <div class="e-codigo-wrap">
                            ...
                            <button type="button" class="e-codigo-btn" onclick="generarCodigo()"
                                    title="Regenerar c&oacute;digo">
                                <i class="fas fa-sync-alt"></i>
                            </button>
                        </div>
                        @error('codigo')<div class="e-error">{{ $message }}</div>@enderror
                        <span class="e-hint">Click en <i class="fas fa-sync-alt"></i> para regenerar</span>
                    </div>
                    ...
                    <div class="e-field">
                        ...
                        <div class="e-toggle-wrap" style="margin-top:.25rem;">
                            <button type="button"
                                    class="e-toggle {{ old('activo', $materia->activo) ? 'on' : '' }}"
                        ...
                        <span class="e-hint">Inactiva: no aparece en asignaciones</span>
                    </div>
                ...
                <div class="e-section-title">
                    ...
                    &Aacute;rea de Conocimiento <span class="req" style="font-size:.7rem;margin-left:.2rem;">*</span>
                </div>
                ...
                <div class="e-field">
                    <textarea name="descripcion"
                              class="e-textarea @error('descripcion') is-invalid @enderror"
                              placeholder="Descripci&oacute;n breve de los objetivos de la materia...">{{ old('descripcion', $materia->descripcion) }}</textarea>
...
function renderAreas() {
    ...
    if (!nivel) {
        grid.innerHTML = '<p style="color:#94a3b8;font-size:.8rem;grid-column:1/-1;">Selecciona un nivel para ver las \u00e1reas disponibles.</p>';
        return;
    }

    areas.forEach(a => {
        const active = a.val === areaSeleccionada;
        const chip = document.createElement('div');
        chip.className = 'area-chip' + (active ? ' active' : '');
