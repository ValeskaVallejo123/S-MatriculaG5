...
@section('topbar-actions')
    ...
    @if(Auth::check() && Auth::user()->user_type === 'profesor')
    <a href="{{ route('horarios.exportPDF') }}"
       class="btn-back"
       style="background: linear-gradient(135deg, #00508f 0%, #003b73 100%);
              color: white; padding: 0.5rem 1.2rem; border-radius: 8px;
              text-decoration: none; font-weight: 600; display: inline-flex;
              align-items: center; gap: 0.5rem; transition: all 0.3s ease;
              border: none; box-shadow: 0 2px 8px rgba(0, 59, 115, 0.3);
              font-size: 0.9rem;">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0" style="color: #003b73;">
                    <thead
                        style="background: linear-gradient(135deg, rgba(78, 199, 210, 0.15),
                        rgba(0, 80, 143, 0.1)); border-bottom: 2px solid #4ec7d2;">
                        <tr>
                            <th>Profesor</th>
                            <th>Día</th>
                            <th>Hora Inicio</th>
                    ...
                    <tbody>
                        ...
                        @forelse($horarios as $horario)
                            @php
                                $estaReciente = $horario->updated_at->diffInHours(now()) < 24;
                            @endphp

                            <tr style="{{ $estaReciente ?
                        ...
                        @empty
                            ...
                            <tr>
                                <td colspan="{{ Auth::check() && Auth::user()->user_type !== 'estudiante' ? 8 : 7 }}"
                                    class="text-center">
