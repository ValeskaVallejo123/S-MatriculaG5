                        ...
                        <tbody>
                            @foreach($horario as $clase)
                                ...
                                <tr style="{{ $resaltado ? 'background-color: rgba(78, 199, 210, 0.1); border-left: 4px solid #4ec7d2;' : '' }}">
                                    <td>{{ $clase->dia }}</td>
                                    <td>{{ \Carbon\Carbon::createFromFormat('H:i:s', $clase->hora_inicio)->format('H:i') }}</td>
                                    <td>{{ \Carbon\Carbon::createFromFormat('H:i:s', $clase->hora_fin)->format('H:i') }}</td>
                                    <td>{{ $clase->materia->nombre ?? '—' }}</td>
                                    <td>{{ $clase->profesor->nombreCompleto ?? '—' }}</td>
                                    <td>{{ $clase->salon ?? '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
