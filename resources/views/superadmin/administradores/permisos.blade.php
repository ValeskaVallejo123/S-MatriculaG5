...
<style>
    .rp-tabs {
        ...
        background: #f1f3f6; padding: 4px; border-radius: 10px;
    ...
    .rp-tab.active { background: #fff; color: #003b73; box-shadow: 0 1px 4px rgba(0,0,0,.1); }
    ...
    .rp-layout { display: grid; grid-template-columns: 250px 1fr; gap: 16px; align-items: start; }
    @media (max-width: 860px) { .rp-layout { grid-template-columns: 1fr; } }

    .rp-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,.05); }

    .rp-sidebar-title { padding: 12px 14px 8px; font-size: .68rem; font-weight: 700; letter-spacing: .09em; text-transform: uppercase; color: #9ca3af; border-bottom: 1px solid #f3f4f6; }
    .rp-search { padding: 10px 12px; border-bottom: 1px solid #f3f4f6; }
    .rp-search input {
        width: 100%; padding: 7px 10px 7px 30px; font-size: .82rem;
    ...
    .rp-search input:focus { border-color: #00508f; box-shadow: 0 0 0 3px rgba(0,80,143,.1); background-color: #fff; }
    ...
    .rp-user-item { display: flex; align-items: center; gap: 10px; padding: 10px 14px; cursor: pointer; border-left: 3px solid transparent; transition: background .15s, border-color .15s; }
    .rp-user-item:hover  { background: #f9fafb; }
    .rp-user-item.active { background: #eff6ff; border-left-color: #00508f; }

    .rp-avatar { width: 34px; height: 34px; border-radius: 50%; font-size: .76rem; font-weight: 700; color: #fff; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .rp-uinfo { overflow: hidden; flex: 1; min-width: 0; }
    .rp-uinfo strong { display: block; font-size: .82rem; font-weight: 600; color: #111827; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .rp-uinfo span    { font-size: .72rem; color: #6b7280; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }
    .rp-rbadge { flex-shrink: 0; font-size: .62rem; font-weight: 700; padding: 2px 7px; border-radius: 20px; }
    .rp-rbadge.sa { background: #fef2f2; color: #dc2626; }
    .rp-rbadge.ad { background: #eff6ff; color: #00508f; }
    .rp-no-results { padding: 20px; text-align: center; font-size: .82rem; color: #9ca3af; display: none; }

    .rp-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 380px; padding: 40px 24px; text-align: center; }
    .rp-empty-icon { width: 52px; height: 52px; background: #f3f4f6; border: 1px solid #e5e7eb; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #9ca3af; font-size: 1.4rem; margin-bottom: 14px; }
    ...
    .rp-panel-head { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; border-bottom: 1px solid #f3f4f6; background: #fafafa; }
    ...
    .rp-sec-label { font-size: .67rem; font-weight: 700; letter-spacing: .09em; text-transform: uppercase; color: #9ca3af; display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
    .rp-sec-label::after { content: ''; flex: 1; height: 1px; background: #f3f4f6; }

    .rp-perms-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
    .rp-link-btn { font-size: .75rem; font-weight: 500; color: #00508f; background: none; border: none; cursor: pointer; padding: 0; font-family: 'Inter', sans-serif; transition: opacity .15s; }
    .rp-link-btn:hover { opacity: .7; }
    .rp-link-btn.red { color: #dc2626; }

    .rp-perms-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(170px, 1fr)); gap: 7px; }
    .rp-perm-item { display: flex; align-items: center; gap: 8px; padding: 9px 11px; border: 1px solid #e5e7eb; border-radius: 7px; cursor: pointer; user-select: none; transition: border-color .15s, background .15s; font-size: .8rem; color: #111827; font-weight: 500; font-family: 'Inter', sans-serif; }
    ...
    .rp-sa-alert { display: flex; gap: 10px; padding: 11px 14px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; font-size: .8rem; color: #92400e; line-height: 1.5; }
    .rp-sa-alert i { color: #f59e0b; margin-top: 1px; flex-shrink: 0; }

    .rp-footer { display: flex; align-items: center; justify-content: flex-end; gap: 8px; padding: 12px 18px; border-top: 1px solid #f3f4f6; background: #fafafa; }
    .rp-btn-cancel { padding: 8px 18px; border-radius: 8px; font-size: .82rem; font-weight: 600; cursor: pointer; border: 1px solid #e5e7eb; background: #fff; color: #6b7280; font-family: 'Inter', sans-serif; transition: background .15s; }
    .rp-btn-cancel:hover { background: #f3f4f6; }
    .rp-btn-save { display: inline-flex; align-items: center; gap: 6px; padding: 8px 20px; border-radius: 8px; font-size: .82rem; font-weight: 600; cursor: pointer; border: none; background: linear-gradient(135deg, #003b73, #00508f); color: #fff; font-family: 'Inter', sans-serif; box-shadow: 0 2px 6px rgba(0,59,115,.25); transition: all .2s; }
    .rp-btn-save:hover { box-shadow: 0 4px 12px rgba(0,59,115,.35); transform: translateY(-1px); }

    .rp-flash-ok  { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 8px; font-size: .83rem; font-weight: 500; margin-bottom: 14px; background: #f0fdf4; border: 1px solid #bbf7d0; color: #15803d; }
    .rp-flash-err { display: flex; align-items: center; gap: 8px; padding: 10px 14px; border-radius: 8px; font-size: .83rem; font-weight: 500; margin-bottom: 14px; background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; }

    .rp-roles-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px; }
    ...
    .rp-role-sm-head { display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: .8rem; font-weight: 700; color: #fff; }
    ...
    .rp-role-row { display: flex; align-items: center; gap: 6px; font-size: .77rem; color: #374151; padding: 3px 0; }
    ...
    .rp-table th { padding: 9px 14px; text-align: left; font-size: .68rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; color: #9ca3af; background: #fafafa; border-bottom: 2px solid #e5e7eb; }
    .rp-table th:not(:first-child) { text-align: center; }
    .rp-table td { padding: 9px 14px; border-bottom: 1px solid #f3f4f6; color: #111827; }
    ...
    .ico-ok { color: #16a34a; }
    .ico-no { color: #d1d5db; }
    .rp-badge-cfg { display: inline-flex; padding: 1px 9px; border-radius: 20px; background: #eff6ff; color: #00508f; font-size: .68rem; font-weight: 700; }

    .rp-restricted { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px 32px; text-align: center; }
</style>
@endpush
...
<div class="rp-pane active" id="rp-tab-config">
    ...
    @if(session('success'))
    <div class="rp-flash-ok"><i class="fas fa-check-circle"></i> {{ session('success') }}</div>
    @endif
    @if($errors->any())
    <div class="rp-flash-err"><i class="fas fa-exclamation-circle"></i> {{ $errors->first() }}</div>
    @endif
        ...
        <div class="rp-layout">
            ...
            <div class="rp-card">
                ...
                <div class="rp-user-list" id="rpUserList">
                    ...
                    <div class="rp-user-item {{ old('usuario_id') == $admin->id ? 'active' : '' }}"
                        ...
                        <span class="rp-rbadge {{ $esSA ? 'sa' : 'ad' }}">{{ $esSA ? 'SA' : 'Admin' }}</span>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="rp-card">
                ...
                <div class="rp-empty" id="rpEmpty">
                    <div class="rp-empty-icon"><i class="fas fa-user-shield"></i></div>
                ...
                <div id="rpActive" class="{{ old('usuario_id') ? '' : 'd-none' }}">
                    ...
                    <div class="rp-footer">
                        ...
                        <button type="submit" class="rp-btn-save">
                            <i class="fas fa-check" style="font-size:.75rem"></i> Guardar cambios
                        </button>
                    </div>
                </div>
            </div>
...
<div class="rp-pane" id="rp-tab-resumen">
    ...
    <div class="rp-roles-grid">
        <div class="rp-role-sm">
            <div class="rp-role-sm-head sa"><i class="fas fa-user-shield"></i> Super Administrador</div>
            <div class="rp-role-sm-body">
                ...
                <div class="rp-role-row"><i class="fas fa-check ok"></i> {{ $c }}</div>
                @endforeach
            </div>
        </div>
        <div class="rp-role-sm">
            <div class="rp-role-sm-head ad"><i class="fas fa-user-tie"></i> Administrador</div>
            <div class="rp-role-sm-body">
