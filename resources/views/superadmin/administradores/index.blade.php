...
@push('styles')
    <style>
        ...
        .del-modal-head {
            background: #fef2f2; padding: 1rem 1.25rem;
            display: flex; align-items: center; gap: .6rem;
        }
        .del-modal-head i { color: #dc2626; font-size: 1.1rem; }
        .del-modal-head h5 { margin: 0; color: #dc2626; font-weight: 700; font-size: 1rem; }
        .del-modal-body { padding: 1.25rem; color: #1e293b; font-size: .9rem; }
        .del-modal-body small { display: block; margin-top: .4rem; color: #94a3b8; }
        .del-modal-foot {
            padding: 1rem 1.25rem; display: flex; justify-content: flex-end; gap: .5rem;
        ...
        .del-btn-cancel { background: #f1f5f9; color: #334155; }
...
@section('content')
    ...
    {{-- ── Modal de confirmación eliminar (sin dependencias de Bootstrap JS) ── --}}
    <div class="del-modal-overlay" id="deleteModalOverlay">
        <div class="del-modal-box">
            ...
            <div class="del-modal-body">
                <p id="deleteModalText" style="margin:0;"></p>
                <small>Esta acción no se puede deshacer.</small>
            </div>
            <div class="del-modal-foot">
                <button type="button" class="del-btn del-btn-cancel" onclick="cerrarModalDelete()">Cancelar</button>
                <form id="deleteForm" method="POST" action="">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="del-btn del-btn-confirm">
...
@push('scripts')
    <script>
        ...
        function mostrarModalDeleteData(btn) {
            ...
            const mensaje = btn.getAttribute('data-message');
            const nombre  = btn.getAttribute('data-name');

            document.getElementById('deleteModalText').textContent = `${mensaje} (${nombre})`;
            document.getElementById('deleteForm').setAttribute('action', ruta);
        ...
        document.getElementById('deleteModalOverlay')?.addEventListener('click', function (e) {
            if (e.target === this) cerrarModalDelete();
        });
    </script>
@endpush
