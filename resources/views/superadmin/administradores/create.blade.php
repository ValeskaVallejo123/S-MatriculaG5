    ...
    <div class="container" style="max-width: 800px;">
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 12px;">
            ...
            <div class="card-body p-4">
                <form action="{{ route('superadmin.administradores.store') }}" method="POST" novalidate>
                    @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="position-relative">
                                    ...
                                    <div class="invalid-feedback" id="nameLiveAlert" style="font-size: 0.8rem; display:none;">
                                        <i class="fas fa-exclamation-circle me-1"></i>El nombre solo puede contener letras y espacios
                                    </div>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="position-relative">
                                    ...
                                    <input type="email"
                                           ...
                                           required
                                           style="border: 2px solid #bfd9ea; border-radius: 8px; padding: 0.6rem 1rem 0.6rem 2.8rem; transition: all 0.3s ease;">
                                    <div class="invalid-feedback" id="emailLiveAlert" style="font-size: 0.8rem; display:none;">
                                        <i class="fas fa-exclamation-circle me-1"></i>El email debe contener @ y un dominio (ej: usuario@dominio.com)
                                    </div>
                                    @error('email')
                                    <div class="invalid-feedback" style="font-size: 0.8rem;">
                                        <i class="fas fa-exclamation-circle me-1"></i>{{ $message }}
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const nameAlert  = document.getElementById('nameLiveAlert');
                const emailInput = document.getElementById('email');
                const emailAlert = document.getElementById('emailLiveAlert');

                // ── Nombre: bloquea la tecla ANTES de que se escriba ──
                if (nameInput) {
                    const letraValida = /^[a-zA-ZÁÉÍÓÚáéíóúÑñÜü\s]$/;

                    nameInput.addEventListener('keydown', function (e) {
                        // Teclas de control siempre permitidas (borrar, mover cursor, copiar, etc.)
                        const teclasControl = ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Tab', 'Home', 'End'];
                        if (teclasControl.includes(e.key) || e.ctrlKey || e.metaKey) {
                            return;
                        }
                        // Si la tecla es un solo carácter y NO es letra/espacio, bloquear
                        if (e.key.length === 1 && !letraValida.test(e.key)) {
                            e.preventDefault();
                    ...
                    // Por si pegan texto con el mouse (Ctrl+V) — limpia lo pegado
                ...
                // ── Email: alerta en vivo si falta el @ ──
                if (emailInput) {
                    const emailRegexCompleto = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                    ...
                    emailInput.addEventListener('input', function () {
                        ...
                        } else if (!emailRegexCompleto.test(valor)) {
                            emailAlert.style.display = 'block';
                            this.classList.add('is-invalid');
                        } else {
                            emailAlert.style.display = 'none';
                    ...
                    emailInput.addEventListener('blur', function () {
                        if (this.value.length > 0 && !emailRegexCompleto.test(this.value)) {
                            emailAlert.style.display = 'block';
                            this.classList.add('is-invalid');
                        }
                    });
