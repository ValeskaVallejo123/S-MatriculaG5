                    ...
                    <div class="col-md-6">
                        <div class="d-flex align-items-center justify-content-md-end gap-3">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="small"><strong style="color: #4ec7d2;">{{ $asignaciones->sum(fn($p) => $p->materiasGrupos->count()) }}</strong> <span class="text-muted">Asignaciones</span></span>
                            </div>
                    ...
                    <table class="table table-hover align-middle mb-0" id="profesorMateriaTable">
                        <thead style="background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%);">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-uppercase small fw-semibold" style="font-size: 0.7rem; color: #003b73;">Grado(s)</th>
                                <th class="px-3 py-2 text-uppercase small fw-semibold" style="font-size: 0.7rem; color: #003b73;">Materias Asignadas</th>
                        ...
                        <tbody>
                            @forelse ($asignaciones as $profesor)
                                <tr class="profesor-row" style="border-bottom: 1px solid #f1f5f9;">
                                    <td class="px-3 py-2">
                                        <div class="d-flex align-items-center gap-2">
                                    ...
                                    <td class="px-3 py-2">
                                        @php
                                            $gradosUnicos = $profesor->materiasGrupos
                                                ->map(fn($mg) => $mg->grado->numero . '° ' . $mg->grado->nombre . ' - Sec. ' . $mg->seccion)
                                                ->unique();
                                        @endphp
                                        <div class="d-flex flex-wrap gap-1">
                                            @forelse($gradosUnicos as $gradoLabel)
                                                <span class="badge" style="background: rgba(0,80,143,0.05); color: #003b73; border: 1px solid #bfd9ea; font-size: 0.7rem; font-weight: 500;">
                                                    <i class="fas fa-layer-group me-1" style="font-size: 0.6rem; opacity: 0.7;"></i>
                                                    {{ $gradoLabel }}
                                                </span>
                                            @empty
                                    ...
                                    <td class="px-3 py-2">
                                        @if($profesor->materiasGrupos->isNotEmpty())
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach($profesor->materiasGrupos as $mg)
                                                    <span class="badge" style="background: rgba(78, 199, 210, 0.1); color: #00508f; border: 1px solid #bfd9ea; padding: 0.25rem 0.5rem; font-size: 0.7rem;">
                                                        <i class="fas fa-book me-1" style="font-size: 0.6rem;"></i> 
                                                        {{ $mg->materia->nombre }}
                                                    </span>
                                                @endforeach
                                            </div>
                                    ...
                                    <td class="px-3 py-2 text-center">
                                        <span class="badge rounded-pill" style="background: rgba(78, 199, 210, 0.2); color: #00508f; border: 1px solid #4ec7d2;">
                                    ...
                                    <td class="px-3 py-2 text-end">
                                        <div class="btn-group">
                                            <a href="{{ route('profesor_materia.edit', $profesor->id) }}" class="btn btn-sm" style="border: 1.5px solid #4ec7d2; color: #4ec7d2; background: white;"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('profesor_materia.destroy', $profesor->id) }}" method="POST" class="d-inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="btn btn-sm" style="border: 1.5px solid #ef4444; color: #ef4444; background: white; border-left: none;"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
