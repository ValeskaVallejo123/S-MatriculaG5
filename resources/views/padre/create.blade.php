            ...
            <div class="form-section">
                ...
                <div class="relation-grid">
                    ...
                    @foreach ($relaciones as $rel)
                        <input type="radio" name="parentesco" id="rel_{{ $rel['value'] }}"
                               value="{{ $rel['value'] }}" class="relation-option"
                               {{ old('parentesco') == $rel['value'] ? 'checked' : '' }}>
                ...
                {{-- Campo visible solo si selecciona "Otro" --}}
                <div id="parentesco_otro_wrap" style="margin-top:1rem; {{ old('parentesco') == 'otro' ? '' : 'display:none;' }}">
                    <div class="field">
                        ...
                        <input type="text" name="parentesco_otro" value="{{ old('parentesco_otro') }}"
                               ...
                               placeholder="Ej: Hermano, Cuñado..." maxlength="50">
                        @error('parentesco_otro') <span class="error-msg">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
...
<script>
    ...
    // Mostrar/ocultar campo "Especificar parentesco"
    document.querySelectorAll('input[name="parentesco"]').forEach(function (radio) {
        radio.addEventListener('change', function () {
            const wrap = document.getElementById('parentesco_otro_wrap');
            wrap.style.display = this.value === 'otro' ? 'block' : 'none';
        });
    });
</script>
