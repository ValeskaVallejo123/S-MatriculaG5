...
<div class="contenedor">
    <h2>Editar período académico</h2>
    ...
    <form method="POST" action="{{ route('periodos-academicos.update', $periodo->id) }}">
        ...
        <input type="text" name="nombre_periodo" id="nombre_periodo" value="{{ old('nombre_periodo', $periodo->nombre_periodo) }}" required>
        ...
        <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $periodo->fecha_inicio) }}" required>
        @error('fecha_inicio') <div class="error">{{ $message }}</div> @enderror

        <label for="fecha_fin">Fecha de fin</label>
        <input type="date" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $periodo->fecha_fin) }}" required>
