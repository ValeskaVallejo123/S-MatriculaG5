...
<style>
    ...
    .pm-tbl-wrapper { overflow-x: auto; -webkit-overflow-scrolling: touch; }
    .pm-tbl { width: 100%; border-collapse: collapse; min-width: 700px; }
    .pm-tbl thead th {
    ...
    .grado-chip i { font-size: .58rem; color: var(--teal); }
    ...
    .materia-chip i { font-size: .58rem; }
    ...
    .mas-chip {
        ...
        border: 1px solid #e2e8f0; margin: .1rem;
    }
    ...
    /* ── Botones acción ── */
    .act-wrap { display: inline-flex; gap: 3px; align-items: center; }
    .act-btn {
        width: 28px; height: 28px; border-radius: 7px;
        display: inline-flex; align-items: center; justify-content: center;
    ...
    .pm-empty p  { font-size: .83rem; color: var(--text-muted); margin: 0 0 1.25rem; }
...
<div>
    ...
    <div class="pm-stats">
        ...
        <div class="pm-stat ps-grupos">
            ...
            <div>
                ...
                <div class="pm-stat-sub">En la lista</div>
            </div>
        </div>
    </div>
    ...
    <div class="pm-toolbar">
        <div class="pm-search-wrap">
            ...
            <input type="text" id="searchInput" class="pm-search"
                   placeholder="Buscar por nombre de profesor, materia o grado...">
        </div>
        <span style="font-size:.82rem;color:var(--text-muted);flex-shrink:0;">
    ...
    <div class="pm-card">
        <div class="pm-card-head">
            ...
            <span>Asignaciones Profesor — Materia — Grado</span>
        </div>

        <div class="pm-tbl-wrapper">
            <table class="pm-tbl" id="pmTable">
                <thead>
                    <tr>
                        ...
                        <th class="tr" style="width:90px;">Acciones</th>
                    </tr>
                </thead>
                <tbody id="tableBody">

                    @forelse($asignaciones as $profesorId => $filas)
                    @php
                        $profesor = $filas->first()->profesor;

                        /* Grados únicos: "1ro Primaria — Sec. A" */
                        $gradosUnicos = $filas->map(function($f) {
                            $g = $f->grado;
                            if (!$g) return null;
                            $num = match((int)$g->numero) {
                                1=>'1ro', 2=>'2do', 3=>'3ro',
                                4=>'4to', 5=>'5to', 6=>'6to',
                                7=>'7mo', 8=>'8vo', 9=>'9no',
                                default => $g->numero.'°'
                            };
                            return $num . ' ' . ucfirst($g->nivel) . ' — Sec. ' . $f->seccion;
                        })->filter()->unique()->values();

                        /* Materias únicas */
                        $materiasUnicas = $filas->map(fn($f) => $f->materia->nombre ?? null)
                            ->filter()->unique()->values();
                        ...
                        $searchText = strtolower(
                            ($profesor->nombre ?? '') . ' ' .
                            ($profesor->apellido ?? '') . ' ' .
                            $gradosUnicos->implode(' ') . ' ' .
                            $materiasUnicas->implode(' ')
                        );

                        /* ID del primer registro (para editar/eliminar el grupo) */
                        $primerIdDelGrupo = $filas->first()->id;
                    @endphp
                    <tr class="pm-row" data-search="{{ $searchText }}">
                        ...
                        <td>
                            <div style="display:flex;align-items:center;gap:.65rem;">
                                ...
                                <div>
                                    <div class="pm-name">{{ $profesor->nombre }} {{ $profesor->apellido }}</div>
                                    @if($profesor->especialidad)
                                        <div class="pm-sub">{{ $profesor->especialidad }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>

                        {{-- Grados ── ahora con datos reales --}}
                        <td>
                            @if($gradosUnicos->isNotEmpty())
                                <div style="display:flex;flex-wrap:wrap;gap:.1rem;">
                                    ...
                                    @if($gradosUnicos->count() > 3)
                                        <span class="mas-chip">+{{ $gradosUnicos->count() - 3 }} más</span>
                                    @endif
                                </div>
                            @else
                        ...
                        {{-- Materias ── con datos reales --}}
                        <td>
                            @if($materiasUnicas->isNotEmpty())
                                <div style="display:flex;flex-wrap:wrap;gap:.1rem;">
                                    @foreach($materiasUnicas->take(4) as $m)
                                        <span class="materia-chip">
                                            <i class="fas fa-book"></i> {{ $m }}
                                        </span>
                                    @endforeach
                                    @if($materiasUnicas->count() > 4)
                                        <span class="mas-chip">+{{ $materiasUnicas->count() - 4 }} más</span>
                                    @endif
                                </div>
                            @else
                        ...
                        {{-- Total asignaciones del profesor --}}
                        <td class="tc">
                            <span class="count-badge">{{ $filas->count() }}</span>
                        </td>

                        {{-- Acciones: editar y eliminar el primer registro del grupo --}}
                        <td class="tr">
                            <div class="act-wrap">
                                <a href="{{ route('profesor_materia_grado.edit', $primerIdDelGrupo) }}"
                                ...
                                <button type="button"
                                        ...
                                        onclick="mostrarModalDelete(
                                            '{{ route('profesor_materia_grado.destroy', $primerIdDelGrupo) }}',
                                            '{{ addslashes(($profesor->nombre ?? '') . ' ' . ($profesor->apellido ?? '')) }}'
                                        )">
        ...
        <div class="pm-footer">
            <span class="pm-footer-info">
                {{ $asignaciones->count() }} {{ $asignaciones->count() == 1 ? 'profesor' : 'profesores' }}
                con {{ $totalAsignaciones }} asignaciones en total
            </span>
        </div>
        @endif
    </div>
