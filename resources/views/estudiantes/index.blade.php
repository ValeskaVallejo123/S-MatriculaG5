...
.est-search-btn:hover { opacity: .88; }
...
.badge-activo   { display:inline-flex;align-items:center;gap:.3rem;padding:.25rem .7rem;border-radius:999px;font-size:.72rem;font-weight:700;background:rgba(78,199,210,.15);color:var(--blue-mid);border:1px solid rgba(78,199,210,.4); }
...
@media(max-width:480px) {
    .est-stats { grid-template-columns: 1fr 1fr; gap: .65rem; }
    .est-stat-icon { width: 42px; height: 42px; font-size: 1.1rem; }
...
<div>
    ...
    <div class="est-stats">
        ...
        <div class="est-stat">
            <div class="est-stat-icon" style="background:rgba(16,185,129,.12);">
        ...
        <div class="est-stat">
            <div class="est-stat-icon" style="background:rgba(245,158,11,.12);">
                <i class="fas fa-user-slash" style="color:var(--amber);"></i>
            </div>
            <div>
        ...
        <div class="est-stat">
            <div class="est-stat-icon" style="background:rgba(139,92,246,.12);">
                <i class="fas fa-user-plus" style="color:var(--purple);"></i>
            </div>
            <div>
                <div class="est-stat-lbl">Nuevos Hoy</div>
                <div class="est-stat-num">{{ $totalHoy }}</div>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('estudiantes.index') }}" id="searchForm">
        ...
        <div class="est-toolbar">
            <div class="est-search-wrap">
                ...
                <input type="text" name="buscar" id="searchInput" class="est-search"
                       placeholder="Buscar por nombre, DNI, grado..."
                       value="{{ request('buscar') }}"
                       autocomplete="off">
            </div>
            <button type="submit" class="est-search-btn">
                <i class="fas fa-search"></i> Buscar
            </button>
            @if(request('buscar'))
            <a href="{{ route('estudiantes.index', ['per_page' => request('per_page', 10)]) }}"
               style="display:inline-flex;align-items:center;gap:.35rem;padding:.5rem .9rem;
                      border-radius:var(--radius-sm);border:1.5px solid var(--border);
                      background:white;color:var(--text-muted);font-size:.82rem;font-weight:600;
    ...
    <div class="est-card">
        <div class="est-card-head">
            <div class="est-card-head-left">
                ...
                <span>Lista de Estudiantes
                    @if(request('buscar'))
                        — "{{ request('buscar') }}"
                    @endif
                </span>
            </div>
            <span class="est-card-count">
                {{ $estudiantes->total() }} {{ request('buscar') ? 'resultados' : 'registros' }}
            </span>
        </div>
            ...
            <table class="est-tbl" id="studentsTable">
                ...
                <tbody id="tableBody">
                    ...
                    <tr>
                        <td colspan="8">
                            <div class="est-empty">
                                <i class="fas fa-{{ request('buscar') ? 'search' : 'user-graduate' }}"></i>
                                @if(request('buscar'))
                                    <h6>Sin resultados para "{{ request('buscar') }}"</h6>
                                    <p>Intenta con otro nombre, DNI o grado</p>
                                    <a href="{{ route('estudiantes.index') }}"
                                       style="display:inline-flex;align-items:center;gap:.5rem;
                                              padding:.55rem 1.3rem;background:white;color:#00508f;
                                              border-radius:9px;text-decoration:none;font-weight:600;
                                              font-size:.85rem;border:1.5px solid #00508f;">
        ...
        <div class="est-footer">
            <span class="est-footer-info">
                ...
                de {{ $estudiantes->total() }}
                {{ request('buscar') ? 'resultados' : 'estudiantes' }}
            </span>
            {{ $estudiantes->links() }}
        </div>
        @endif
