...
@push('styles')
    <style>
        ...
        .pagination .page-link:hover {
            background: #bfd9ea; border-color: #4ec7d2; color: #003b73;
        }
        ...
        .form-control-sm, .form-select-sm {
            border-radius: 6px; border: 2px solid #bfd9ea;
            padding: 0.5rem 0.75rem; transition: all 0.3s ease; font-size: 0.875rem;
        }
                    ...
                    <div class="row align-items-end g-2">
                        ...
                        <div class="col-md-3 d-flex gap-2">
                            <button type="submit" class="btn btn-sm fw-semibold flex-fill"
                                    style="background: linear-gradient(135deg, #4ec7d2 0%, #00508f 100%); color: white; border: none; border-radius: 8px;">
                                <i class="fas fa-filter me-1"></i>Filtrar
                            </button>
                            <a href="{{ route('observaciones.index') }}" class="btn btn-sm fw-semibold flex-fill"
                               style="border: 2px solid #00508f; color: #00508f; background: white; border-radius: 8px;">
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead style="background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%);">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-uppercase small fw-semibold" style="font-size: 0.7rem; color: #003b73;">Descripción</th>
                                <th class="px-3 py-2 text-uppercase small fw-semibold" style="font-size: 0.7rem; color: #003b73;">Tipo</th>
                                <th class="px-3 py-2 text-uppercase small fw-semibold" style="font-size: 0.7rem; color: #003b73;">Fecha</th>
                            ...
                            <tr style="border-bottom: 1px solid #f1f5f9; transition: all 0.2s ease;">
                                <td class="px-3 py-2">
                                    <div class="fw-semibold" style="color: #003b73; font-size: 0.9rem;">
                                        {{ $obs->estudiante?->nombreCompleto ?? 'Sin estudiante' }}
                                    </div>
                                </td>
                                <td class="px-3 py-2">
                                    <span class="small" style="color: #00508f; font-size: 0.85rem;">
                                        {{ $obs->profesor?->nombreCompleto ?? 'Sin asignar' }}
                                    </span>
                                </td>
                                <td class="px-3 py-2">
                                    <span class="small" style="color: #003b73; font-size: 0.85rem;">
                                        {{ $obs->descripcion ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-3 py-2">
                                    @if($obs->tipo === 'positivo')
                                        <span class="badge rounded-pill"
                                              style="background: rgba(76,175,80,0.2); color: #2e7d32; padding: 0.3rem 0.7rem; font-weight: 600; border: 1px solid #4caf50; font-size: 0.75rem;">
                                            <i class="fas fa-circle" style="font-size: 0.4rem; color: #4caf50;"></i> Positivo
                                        </span>
                                    @elseif($obs->tipo === 'negativo')
                                        <span class="badge rounded-pill"
                                    ...
                                    @else
                                        <span class="badge rounded-pill"
                                              style="background: #f1f5f9; color: #64748b; padding: 0.3rem 0.7rem; font-weight: 600; border: 1px solid #cbd5e1; font-size: 0.75rem;">
                                            Sin tipo
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-3 py-2 text-end">
                                    <div class="btn-group" role="group">
                                        ...
                                        <form action="{{ route('observaciones.destroy', $obs) }}" method="POST"
                                              class="d-inline"
                                ...
                                <td colspan="6" class="text-center py-5">
                                    <div class="text-muted">
                                        ...
                                        <a href="{{ route('observaciones.create') }}" class="btn btn-sm"
                                           style="background: linear-gradient(135deg, #4ec7d2 0%, #00508f 100%); color: white; border-radius: 8px; padding: 0.5rem 1.2rem; text-decoration: none; display: inline-block;">
