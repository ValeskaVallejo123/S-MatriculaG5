...
.obs-header-left { display:flex; align-items:center; gap:1rem; }
.obs-header-icon {
    width:46px; height:46px; background:rgba(78,199,210,.25);
    border-radius:11px; display:flex; align-items:center;
    justify-content:center; flex-shrink:0;
}
.obs-header h5 { color:#fff; font-weight:700; margin:0 0 .15rem; font-size:1.05rem; }
.obs-header p  { color:rgba(255,255,255,.7); margin:0; font-size:.8rem; }
.obs-id-badge {
    background:rgba(78,199,210,.2); color:#fff;
    padding:.3rem .8rem; border-radius:7px; font-size:.78rem; font-weight:700;
    white-space:nowrap;
}
...
.student-selector.selected { border-style:solid; border-color:var(--cyan); background:rgba(78,199,210,.08); }
...
.modal-search { padding:.85rem 1rem; border-bottom:1px solid #e2e8f0; position:relative; }
.modal-search i { position:absolute; left:1.75rem; top:50%; transform:translateY(-50%); color:var(--blue-mid); font-size:.85rem; }
.modal-search input {
...
.modal-empty { text-align:center; padding:3rem 1rem; color:#94a3b8; }
...
<div class="obs-header">
    <div class="obs-header-left">
        ...
        <div>
            <h5>Editar Observación</h5>
            <p>Modifique la información de la observación</p>
        </div>
    </div>
    <div class="obs-id-badge">ID: #{{ $observacion->id }}</div>
</div>
    ...
    <form action="{{ route('observaciones.update', $observacion) }}" method="POST">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="student-selector selected"
                         id="studentSelector" onclick="abrirModal()">
                        <div class="student-selector-icon active" id="selectorIcon">
                            <i class="fas fa-user-graduate"></i>
                        </div>
                        <div class="student-selector-text">
                            <div id="selectorHint" class="hint" style="display:none;">
                                <i class="fas fa-mouse-pointer me-1"></i>
                                Haz clic para buscar y seleccionar un estudiante
                            </div>
                            <div id="selectorName" class="name">
                                {{ $observacion->estudiante->nombreCompleto ?? '—' }}
                            </div>
                            <div id="selectorSub" class="sub">
                                <i class="fas fa-graduation-cap me-1" style="color:#4ec7d2;"></i>
                                {{ $observacion->estudiante->grado ?? '' }}
                                — Sección {{ $observacion->estudiante->seccion ?? '' }}
                                &nbsp;·&nbsp;
                                <i class="fas fa-id-card me-1" style="color:#4ec7d2;"></i>
                                {{ $observacion->estudiante->dni ?? '—' }}
                            </div>
                        </div>
                    ...
                    <input type="hidden" name="estudiante_id" id="estudianteId"
                           value="{{ old('estudiante_id', $observacion->estudiante_id) }}">
                        ...
                        <select name="profesor_id" class="field-input" style="padding-left:2.6rem;">
                            ...
                            @foreach($profesores as $prof)
                                <option value="{{ $prof->id }}"
                                    {{ old('profesor_id', $observacion->profesor_id) == $prof->id ? 'selected' : '' }}>
                                    {{ $prof->nombreCompleto }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="tipo-grid">
                        ...
                        <div>
                            <input type="radio" class="tipo-btn" name="tipo"
                                   ...
                                   {{ old('tipo', $observacion->tipo) === $t['value'] ? 'checked' : '' }}>
            ...
            <div class="col-lg-6 d-flex flex-column">
                ...
                <div class="desc-wrap flex-fill d-flex flex-column">
                    ...
                    <textarea name="descripcion" id="descripcion"
                              ...
                              maxlength="1000"
                              placeholder="Describe la observación con detalle...">{{ old('descripcion', $observacion->descripcion) }}</textarea>
                </div>
        ...
        <div style="display:flex;gap:.75rem;padding-top:1.25rem;margin-top:.5rem;border-top:1px solid #e2e8f0;">
            <button type="submit" class="btn-submit">
                <i class="fas fa-save"></i> Actualizar Observación
            </button>
...
<div class="modal-overlay" id="modalEstudiante" onclick="cerrarModalClick(event)">
    <div class="modal-box">
        ...
        <div class="modal-filters" id="modalFilters">
            ...
            @php $gradosUnicos = $estudiantes->pluck('grado')->unique()->filter()->sort()->values(); @endphp
        ...
        <div class="modal-foot">
            ...
            <span><i class="fas fa-info-circle me-1"></i>Haz clic para seleccionar</span>
        </div>
...
let filtroGrado = '';
let estudianteSeleccionado = todosEstudiantes.find(e => e.id === {{ $observacion->estudiante_id ?? 'null' }}) ?? null;
...
function renderLista(lista) {
    ...
    container.innerHTML = lista.map(e => {
        ...
        const avatar   = e.foto ? `<img src="${e.foto}" alt="">` : initials;
        return `
        <div class="modal-student-item ${isActive}"
             onclick="seleccionarEstudiante(${JSON.stringify(e).replace(/"/g,'&quot;')})">
            ...
            <span class="s-badge">${e.grado} ${e.seccion}</span>
        </div>`;
    }).join('');
}
...
function seleccionarEstudiante(est, cerrar = true) {
    ...
    document.getElementById('selectorIcon').classList.add('active');
