...
class MatriculaController extends Controller
{
    ...
    public function edit(Matricula $matricula)
    {
        ...
        return view('matriculas.edit', compact('matricula', 'grados', 'secciones', 'parentescos'));
    }
    ...
    public function confirmar(Matricula $matricula)
    {
        if ($matricula->estado === 'pendiente') {
            try {
                ...
                return redirect()->back()->with('success', 'La matrícula ha sido aprobada correctamente.');
            } catch (\Exception $e) {
                return redirect()->back()->withErrors(['error' => 'Error al aprobar la matrícula.']);
            }
        }
    ...
    public function rechazar(Request $request, Matricula $matricula)
    {
        ...
        if ($matricula->estado === 'pendiente') {
            try {
                ...
                return redirect()->back()->with('success', 'La matrícula ha sido rechazada.');

            } catch (\Exception $e) {
                return redirect()->back()->withErrors(['error' => 'Error al rechazar la matrícula.']);
            }
        }
    ...
    public function cancelar(Matricula $matricula)
    {
        try {
            ...
            return redirect()->back()->with('success', 'La matrícula ha sido cancelada.');
        } catch (\Exception $e) {
            return redirect()->back()->withErrors(['error' => 'Error al cancelar la matrícula.']);
        }
    }
}
