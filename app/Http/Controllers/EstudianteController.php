...
class EstudianteController extends Controller
{
    ...
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            // Añadimos el ID 4 a la lista de permitidos
    ...
    public function misNotas()
    {
        ...
        // Traemos al estudiante con sus notas, materias y periodos
        $estudiante = Estudiante::with(['calificaciones.materia', 'calificaciones.periodo'])
            ->where('email', $user->email)
            ->first();

        if (!$estudiante) {
            return redirect()->back()->with('error', 'No se encontró perfil de estudiante.');
        }

        // Si tu archivo se llama Notas.blade.php y está en resources/views/estudiante/
        return view('estudiante.Notas', compact('estudiante'));
    }
}
